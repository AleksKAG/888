...
func TestAddGetDelete(t *testing.T) {
	...
	require.NotEqual(t, 0, id, "Идентификатор посылки должен быть не нулевым")
	...
	parcel.Number = id
	assert.Equal(t, parcel, storedParcel, "Не совподают значения")
...
func TestSetAddress(t *testing.T) {
	...
	storedParcel, err := store.Get(id)
	require.NoError(t, err, "Ошибка при получении посылки")
	require.Equal(t, newAddress, storedParcel.Address, "Адрес не был обновлен")
}
...
func TestSetStatus(t *testing.T) {
	...
	storedParcel, err := store.Get(id)
	require.NoError(t, err, "Ошибка при получении посылки")
	require.Equal(t, newStatus, storedParcel.Status, "Статус не был обновлен")
}
...
func TestGetByClient(t *testing.T) {
	...
	require.Equal(t, len(parcels), len(storedParcels), "Неверное количество посылок")
	...
	for _, parcel := range storedParcels {
		...
		require.True(t, ok, "Посылки нет в parcelMap")
